...
Ignoring differences in environmental conditions between seasons, there are no significant differences between the number of sightings/survey between seasons.
...
\caption{Sightings at different tide levels}

\paragraph{Relationship of shark and dolphin sightings to baitfish}

It is logical that dolphin and shark sightings may be seen near baitfish schools as they are an important food source. As the transect width (300m) is negligable compared to the transect length (~250km), only the latitudinal distance between sightings is calculated. The distance is calculated using the Haversine function. 
...
\includegraphics{fishWithin10km}
...
The swim direction of all dolphin and shark sightings is noted using North-East-South-West or Circling. With the exception of migrating whales, there is not an expectation of which direction the animals will be swimming. No difference was seen in swim direction between north and southbound flights.
...
Bottlenose dolphin group size is estimated for all sightings. A histogram of group sizes shows exponentially decreasing group size, with most containing <10 individuals.
