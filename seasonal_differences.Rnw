...
The size of the closest fish school might be important. Given the few number of white shark sightings, only the closest fish school size (Small, Medium or Large) to bottlenose dolphins is shown below. Sightings when the closest fish sighting is >5km away are not shown. Note: Both north and southbound flights are pooled for this plot. Most closest fish schools are Small, however as most fish schools seen overall are also Small this does not indicate a preference for small fish schools.
...
Group sizes are very similar for Spring and Summer but may be slightly lower during Autumn. Even if the number of hours surveyed during each season differs, the distribution of group size should be the same (although overall number of sightings might be different). Given the difficulty in accurately estimating dolphin group size (especially for larger groups) the small difference in Autumn compared to the other seasons is likely not notable.
...
\begin{figure}
\centering
\includegraphics{botGroupSizeSeason}\\
