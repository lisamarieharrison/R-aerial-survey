...
for (i in 1:nrow(dat)) {
  for (j in 1:nrow(dat)) {
    if(dat$Flight.Direction[i] == dat$Flight.Direction[j]) {
      if (dat$Date[i] == dat$Date[j]) {
...
The size of the closest fish school might be important. Given the few number of white shark sightings, only the closest fish school size (Small, Medium or Large) to bottlenose dolphins is shown below. Note: Both north and southbound flights are pooled for this plot.

<<closestFishSize, echo=false, results=hide>>=

min_dist_to_fish <- apply(distance[bot, bait], 1, which.min)
closest_size <- dat_season_north$Length.Size[dat_season_north$Species == "B"][unlist(min_dist_to_fish)]
