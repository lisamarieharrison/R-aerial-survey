...
The significance of these differences was assessed separately for each species because the patterns across seasons are not condistent across species. The table below shows the p-values. There were significantly more hammerhead and white shark sightings in Summer compared to the other two seasons. There were also more bottlenose dolphin sightings in Summer and Spring compared to Autumn. White the differences in baitfish sightings were not significant, there may be a significant difference that might not be detectable due to insufficient power in sampling design or analysis.\\*
...
hh <- lmTest(sightings_tab, "HH")
w <- lmTest(sightings_tab, "W")

p_tab <- rbind(bait, bot, hh, w)
colnames(p_tab) <- c("Spring", "Summer")

kable(p_tab, format = "pandoc", caption = "P-values for seasonal differences in the number of sightings/survey for baitfish, bottlenose dolphins, hammerhead and white sharks")
