...
dat <- read.csv("environmental_effort.csv", header = T)[1:2431, ]
library(chron)
...
envt.var.north <- matrix(0, ncol = 23)
...
for (i in unique(dat.north$Date)) {
  for (j in 1:(nrow(dat.north[dat.north$Date == i, ]) - 1)) {
    ...
    for (k in c(5, 8, 9, 11)) {
      w <- which(colnames(envt.var.north) == as.name(paste(names(dat.north)[k], ".north.", dat[dat.north$Date == i, ][j, k], sep = "")))
...
for (i in unique(dat.south$Date)) {
  for (j in 1:(nrow(dat.south[dat.south$Date == i, ]) - 1)) {
    ...
    #add minute differences to each environmental level
    for (k in c(5, 8, 9, 11)) {
      w <- which(colnames(envt.var.south) == as.name(paste(names(dat.south)[k], ".south.", dat[dat.south$Date == i, ][j, k], sep = "")))
